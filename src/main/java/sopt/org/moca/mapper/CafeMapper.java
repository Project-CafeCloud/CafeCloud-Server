...
public interface CafeMapper {
    ...
    @Select("select cafe_id, cafe_address_district,cafe_name , address_district, evaluated_cafe_img_url" +
            "from EVALUATED_CAFE natural join EVALUATED_CAFE_IMG natural join CAFE natural join ADDRESS_DISTRICT limit = #{length}")
    List<CafeSimple>findPopularEvaluatedCafe(@Param("length") final int length);

    //인기 있는 검증 카페 리스트 조회(전체)
    @Select("select cafe_id, cafe_address_district,cafe_name , address_district, evaluated_cafe_img_url,evaluated_cafe_rating" +
            "from EVALUATED_CAFE natural join EVALUATED_CAFE_IMG natural join CAFE natural join ADDRESS_DISTRICT")
    List<CafeSimple>findAllEvaluatedCafe();

    //검증 카페 상세 정보 조회(카페이름, 카페주소,총평)
    @Select("select cafe_name, cafe_address, evaluated_cafe_total_evaluation " +
    ...
    @Select("select barista_name,barista_title,barista_img_url,evaluation_bean_condition,evaluation_bean_condition_comment,evaluation_roasting,evaluation_roasting_comment,evaluation_creativity,evaluation_creativity_comment,evaluation_reasonable,evaluation_reasonable_comment,evaluation_consistancy,evaluation_consistancy_comment,evaluation_summary" +
            "from EVALUATION natural join BARISTA" +
            "where barista_id = #{barista_id}")
    Evaluatuion_detail findBaristaEvaluation(@Param("barista_id")final int barista_id);
